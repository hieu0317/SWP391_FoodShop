...
public class BlogUpdate extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        request.setAttribute("blog", bd.get(id));
        request.getRequestDispatcher("views/BlogUpdate.jsp").forward(request, response);
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        int id = Integer.parseInt(request.getParameter("id"));
        blogdao.updateBlog(title, detail, id);
        //add image file
        for (Part part : Parts) {
            String fileName = part.getSubmittedFileName();
            if (fileName != null) {
                part.write(projectPath + "images\\blogimage\\" + fileName);
                blogdao.UpdateBlogImg(fileName, id);
            }
        }
        //
